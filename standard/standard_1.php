 ...
 * If <b>needle</b> is not a string, <br>
 * it is converted to an integer and applied as the ordinal value of a character. -> <br>
 * 不是字符串尝试将其 整数->对应字符
 * </p>
 * @param int $offset [optional] <p>
 * If specified, search will start this number of characters counted from the beginning of the string. <br>
 * Unlike {@see strrpos()} and {@see strripos()}, the offset cannot be negative. -> 不能为负数
 * @return int|boolean <p>
 * Returns the position where the needle exists relative to the beginning of
 * the <b>haystack</b> string (independent of search direction or offset). <br>
 * Also note that string positions start at 0, and not 1. <br>
 * Returns <b>FALSE</b> if the needle was not found. ->   找不到 return false 不是常见其他语言的 -1
 * @since 4.0
 */
...
/**
 * Find position of first occurrence of a case-insensitive string -> <br>
 * strpos 的 i 版本(case insensitive)
 ...
 * </p>
 * @return int|boolean If needle is not found, stripos will return boolean false.
 * @see strpos
 * @since 5.0
 */
 ...
 * strpos 的 right 版本 注意可以为负数(offset)
...
/**
 * Find position of last occurrence of a case-insensitive string in a string -> <br>
 * strrpos 的 i 版本(case insensitive)
...
/**
 * Convert logical Hebrew text to visual text ->  貌似处理希伯来文的用处不大
...
/**
 * Convert logical Hebrew text to visual text with newline conversion ->  貌似处理希伯来文的用处不大
...
/**
 * Inserts HTML line breaks before all newlines in a string -> 在字符串所有新行之前插入 HTML 换行标记 <br>
 * It's important to remember that this function does NOT replace newlines with &lt;br&gt; tags. <br>
 * Rather, it inserts a &lt;br&gt; tag before each newline, but it still preserves the newlines themselves! <br>
 *  如果要替换新行可以考虑 <code>str_replace( "\n", '<br />', $Text ); </code> <br>
 *  所以他的名字非常误导人:new line to break...
 ...
 * @return string the altered string, with &lt;br /&gt; or &lt;br&gt; inserted before all newlines (\r\n, \n\r, \n and \r).
 * @since 4.0
 */
...
/**
 * Returns filename component of path -> 返回路径中的文件名部分
 * @link http://php.net/manual/en/function.basename.php
 * @param string $path A path. <br>
 * On Windows, both slash (/) and backslash
 * (\) are used as directory separator character. <br>
 * In other environments, it is the forward slash (/). ->  注意和平台有关
 * @param string $suffix [optional] <p>
 * If the filename ends in suffix this will also be cut off. -> <br>
